...
(★★★) Una función lineal se define como f(x) = ax + b. 
Si tenemos dos funciones lineales f_1(x) = a_1x + b_1 y f_2(x) = a_2x + b_2, entonces la composición puede simplificarse a: f_3(x) = f_1(f_2(x)) = (a_1.a_2 )x + (a_1.b_2 + b_1). 
Es decir, tenemos una nueva función lineal, cuyos a y b son los resultates marcados.

Utilizando división y conquista, implementar una función composición_n(a, b, c, n) que reciba los valores de a y b de una función lineal, c y n y determine el valor de f^n(c) = f(f(f(…f(c)) (n composiciones de la función f consigo misma) en tiempo O(log n). 
...
* Si n es par, entonces los los valores de a y b corresponden a los de f^{n/2}(f^{n/2}(x)). 
* Si n es impar, los valores de a y b son los que corresponden a los de f(f^{n-1}(x)).

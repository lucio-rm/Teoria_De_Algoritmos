...
- No hay que tocar el algoritmo interno. Uso EXACTAMENTE la misma función de 
...
def _candidato_mayoritario(arr):
    ...
    pares_iguales = []
    # emparejo de a dos. Si son iguales, sobrevive uno a la siguiente ronda.
    for i in range(0, len(arr) - 1, 2):
        if arr[i] == arr[i+1]:
            pares_iguales.append(arr[i])

    # Una sola llamada recursiva sobre un arreglo de tamaño <= n/2
    candidato = _candidato_mayoritario(pares_iguales)

    # verifico si el candidato de las rondas superiores es mayoría aquí
...
La complejidad recae netamente en _candidato_mayoritario(arr), que ya 

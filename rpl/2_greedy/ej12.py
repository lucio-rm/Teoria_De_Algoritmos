...
def asignar_mafias(pedidos):
    ...
    # Regla Greedy: Ordenamos de menor a mayor por el kilómetro de fin (posición 1 de la tupla)
    # hago un sorted para no mutar la lista original de la cátedra
    pedidos_ordenados = sorted(pedidos, key=lambda columna: columna[1])
    
    mafias_aprobadas = []
    # rastrea hasta qué kilómetro está ocupada la ruta costera actualmente
    km_ocupado_hasta = -1.0 
    
    for pedido in pedidos_ordenados:
        km_inicio = pedido[0]
        km_fin = pedido[1]
        
        # si el rango pedido empieza después (o igual) de donde terminó la última mafia
        if km_inicio >= km_ocupado_hasta:
            mafias_aprobadas.append(pedido)
            km_ocupado_hasta = km_fin # actualizo el límite del control territorial

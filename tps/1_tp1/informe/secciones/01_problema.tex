...
\begin{itemize}
    \item ¿Cómo se podría hacer que eligiendo $x$ moneda (una entre esas dos), no haya otra moneda próxima (mayor a $x$) que otro la elija en el turno siguiente? 
    \item ¿Cómo es posible crear un algoritmo Greedy en el cual sea óptimo para cualquier caso de monedas? ¿Qué es un algoritmo Greedy y qué significa que sea óptimo?
...
Hacer que gane siempre Sophia, y tenemos la suerte también de que Sophia es la que elige por Mateo. Entonces, cómo podemos aconsejar a Sophia de tal forma que si siempre hace lo que le hayamos dicho, gane siempre?
...
Pero, ¿Qué tiene que elegir Mateo? ¿Qué tiene que elegir \textit{Sophia} por Mateo?

Para que Sophia gane, tiene que tener la mayor cantidad de valor acumulado comparado con Mateo, entonces si Sophia agarra por cada par siempre la mayor, Sophia va a agarrar \textit{por Mateo} la menor. Siempre para cada par de monedas, Sophia agarra la mayor para ella y para Mateo va a agarrar la menor entre estas dos.
...
\begin{itemize}
    ...
    \item Va a haber que demostrar matemáticamente que es óptimo y no gana siempre.
...
A eso sumarle el pensamiento Greedy, hasta acá se dijo sutilmente como es éste. A continuación, se explica exáctamente eso: qué es un \textbf{algoritmo Greedy}, por qué se lo llama así, y más que nada cómo se puede implementar a este problema propuesto.
